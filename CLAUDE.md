...
6. **About** — a Quick Tools menu item that shows build version and date (plus author) in a `MessageBox`.
   Version comes from `source.extension.vsixmanifest`'s `Identity/@Version`; date is the real compile date.
   Both are baked in at build time into `BuildInfo.g.cs`, generated by an MSBuild target in
   `SsmsQuickTools.csproj` (the manifest is a `None` item, so nothing reads it at runtime otherwise).
...
`ExpandTokenCommand`, `AboutCommand` — plus `AutoReplacementService`, which isn't a command but attaches
the editor command filter.
...
- `About/` — `AboutCommand` shows a `MessageBox` with `BuildInfo.Version`/`BuildInfo.Date` (from
  `BuildInfo.g.cs`, generated by the `GenerateBuildInfo` MSBuild target in `SsmsQuickTools.csproj` from
  `source.extension.vsixmanifest`'s `Identity/@Version` and the real compile date) and the author.
